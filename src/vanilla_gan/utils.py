...
def get_device():
    ...
    return torch.device(
        "cuda:0" if torch.cuda.is_available() and config["TRAIN_ON_GPU"] else "cpu"
    )
...
def save_models(generator, discriminator, epoch):
    ...
    torch.save(
        generator.state_dict(), f"{data_dir}/runs/{UNIQUE_RUN_ID}/generator_{epoch}.pth"
    )
    torch.save(
        discriminator.state_dict(),
        f"{data_dir}/runs/{UNIQUE_RUN_ID}/discriminator_{epoch}.pth",
    )
...
def print_training_progress(batch, generator_loss, discriminator_loss):
    ...
    print(
        f"Losses after mini-batch {batch:5d}: generator {generator_loss:e}, discriminator {discriminator_loss:e}"
    )
