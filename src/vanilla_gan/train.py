...
def perform_train_step(
    ...
    real_images = real_data[0].to(
        device
    )  # retrieve the image of (image, label) from the original MNIST dataset
    ...
    error_real_images = forward_and_backward(
    ...
    error_generated_images = forward_and_backward(
...
def main():
    ...
    generator_optimizer, discriminator_optimizer = initialize_optimizers(
